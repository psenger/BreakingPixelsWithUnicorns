...
router.get('/', (req, res, next) => {
    ...
    (async () => {
        const filePathAndName = path.join(__dirname, '..', 'images', 'RunningFox.jpg');
        const newFileName = path.join(__dirname, '..', 'images', `_${path.parse(filePathAndName).name}${path.parse(filePathAndName).ext}`);
        try {
            const image = await jimp.read(filePathAndName);
            image.scaleToFit(50, 50);
            await image.writeAsync(newFileName);

            const pixels = await getPixels(newFileName);
            const {data, shape, stride, offset} = pixels;
